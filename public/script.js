...
function playEpisode(id, ep) {
  ...
  // Update link server ke yang paling stabil (2026)
  const servers = {
  ...
  // Pastikan ID container dan iframe sinkron
  container.innerHTML = `
...
// Taruh fungsi ini di luar biar bisa dipanggil tombol
window.changeServer = function(url) {
  const player = document.getElementById("main-player");
  if (player) {
    player.src = url;
    console.log("Pindah server ke:", url);
  } else {
    console.error("Iframe player gak ketemu!");
  }
};
